{% macro apply_meta_as_tags_modified(results) %}
  {% if execute %}
    {#
    ...
    {% for res in results -%}
        {% if model_contains_tag_meta(res.node) %}
            ...
            {% if tag_schema_full not in tags_by_schema.keys() %}
                {{ log('need to fetch new tags for schema '+tag_schema_full, info=True) }}
            ...
            #}
            {%- call statement('main', fetch_result=True) -%}
                select LEVEL,OBJECT_NAME,COLUMN_NAME,UPPER(TAG_NAME) as TAG_NAME,TAG_VALUE from table({{model_database}}.information_schema.tag_references_all_columns('{{model_alias}}', 'table'))
            {%- endcall -%}
            ...
            {% for table_tag in model_meta.database_tags %}
                {{ create_tag_if_missing(tag_schema_full|upper, current_tags_in_schema,table_tag|upper) }}
                {% set desired_tag_value = model_meta.database_tags[table_tag] %}
                {{ set_table_tag_value_if_different(tag_schema_full|upper, model_alias|upper,table_tag,desired_tag_value,existing_tags_for_table) }}
            {% endfor %}
            {% for column in res.node.columns %}
                {% for column_tag in res.node.columns[column].meta.database_tags %}
                    {{log(column_tag,info=True)}}
                    {{ create_tag_if_missing(tag_schema_full|upper, current_tags_in_schema,column_tag|upper)}}
                    {% set desired_tag_value = res.node.columns[column].meta.database_tags[column_tag] %}
                    {{ set_column_tag_value_if_different(tag_schema_full|upper, model_alias|upper,column|upper,column_tag,desired_tag_value,existing_tags_for_table)}}
                {% endfor %}
            {% endfor %}
...
{% macro create_tag_if_missing(tag_schema_full, all_tag_names,new_tag) %}
	{% if new_tag not in all_tag_names %}
        ...
        {%- call statement('main', fetch_result=True) -%}
            create tag if not exists {{tag_schema_full}}.{{new_tag}}
        {%- endcall -%}
...
{% macro set_column_tag_value_if_different(tag_schema_full, table_name,column_name,tag_name,desired_tag_value,existing_tags) %}
    ...
    {% else %}
        {{ log('Setting tag value for '+tag_name+' to value '+desired_tag_value, info=True) }}
        {%- call statement('main', fetch_result=True) -%}
            alter table {{table_name}} modify column {{column_name}} set tag {{tag_schema_full}}.{{tag_name}} = '{{desired_tag_value}}'
        {%- endcall -%}
        {{ log(load_result('main').data, info=True) }}
    {% endif %}
{% endmacro %}
